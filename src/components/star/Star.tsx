...
function Star({ roomId }: Props) {
  ...
  const filledStars = Array(5).fill("gold", 0, goldStar).fill("gray", goldStar);

  useEffect(() => {
    IIFE(async () => {
      try {
        const commentAPI = await getCommentRoomId(String(roomId));
        const result = commentAPI.content.filter(
          (comment: any) => comment.saoBinhLuan <= 5,
        );
        const allStars = result.reduce(
          (total: number, comment: any) => total + comment.saoBinhLuan,
          0,
        );
  ...
  return (
    <div className="stars mb-1 flex items-center sm:text-[1.2rem] md:text-[1.5rem]">
